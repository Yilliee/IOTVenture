...
class DBOps_Wrapper {
  ...
  async logoutUser(username, password) {
    try {
      ...
      if (0 === countResult.count) {
        return "SESSIONS_EMPTY";
      }
      console.log("going to run delete");
      ...
      console.log("delete run success!");
      return "SUCCESS";
    } catch (err) {
...
class PlayerController {
  ...
  async logoutUser(req, res) {
    ...
    try {
      const ret_val = await this.db_wrapper.logoutUser(username, password);
      if (ret_val === "SUCCESS") return res.status(200).json({});
      
      const error_map = {
        "INVALID_USERNAME": "User not found",
        "INCORRECT_PASSWORD": "Incorrect password",
        "SESSIONS_EMPTY": "All sessions are already logged out for this team!"
      };
